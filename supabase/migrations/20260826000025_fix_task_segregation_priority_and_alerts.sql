-- Migration: 20260826000025_fix_task_segregation_priority_and_alerts.sql
-- Description: Fix task_priority enum casting in segregate_task RPC and add department-wise upward delegation and Founder alerts
...
BEGIN
    ...
    v_dept_name := COALESCE(v_parent.department_name, 'General');
    ...
    LOOP
        ...
        -- Map and sanitize priority to valid task_priority enum values ('Low', 'Medium', 'High')
        v_priority := CASE 
            WHEN UPPER(TRIM(COALESCE(v_child_item->>'priority', ''))) IN ('HIGH', 'URGENT') THEN 'High'
            WHEN UPPER(TRIM(COALESCE(v_child_item->>'priority', ''))) = 'LOW' THEN 'Low'
            ELSE 'Medium'
        END;
        ...
        -- Insert child task with explicit task_priority casting
        ...
        IF v_assignee_id IS NOT NULL THEN
            ...
            -- Department-scoped notification to new assignee
            IF v_actor.role = 'Employee' AND v_assignee.role IN ('Manager', 'Department Head') THEN
                -- Upward delegation alert when employee requests supervisor assistance
                INSERT INTO public.in_app_notifications (
                    ...
                    v_assignee_id,
                    'Upward Delegation Request',
                    format('%s - Employee %s decomposed subtask "%s" to you for assistance (parent: "%s").', v_dept_name, COALESCE(v_actor.full_name, v_actor.email), v_title, v_parent.title),
                    'TASK_ASSIGNED',
            ...
            ELSE
                INSERT INTO public.in_app_notifications (
                    ...
                    v_assignee_id,
                    'New Task Assigned',
                    format('%s - Assigned execution task "%s" by %s (subtask of "%s").', v_dept_name, v_title, COALESCE(v_actor.full_name, v_actor.email), v_parent.title),
                    'TASK_ASSIGNED',
                    format('/task/%s', v_child_id)
                );
            END IF;
    ...
    INSERT INTO public.audit_logs (
        ...
        v_actor.id,
        'TASK_UPDATE',
        format('[%s] Task "%s" segregated into %s subtasks by %s (%s). Subtasks: %s', v_dept_name, v_parent.title, v_created_count, COALESCE(v_actor.full_name, v_actor.email), v_actor.role, v_child_names_summary)
    );
    ...
    IF v_parent.created_by IS NOT NULL AND v_parent.created_by != v_actor.id THEN
        INSERT INTO public.in_app_notifications (
            ...
            v_parent.created_by,
            'Task Segregated',
            format('%s - %s segregated "%s" into %s execution tasks: %s', v_dept_name, COALESCE(v_actor.full_name, v_actor.email), v_parent.title, v_created_count, v_child_names_summary),
            'TASK_SEGREGATED',
            format('/task/%s', p_parent_task_id)
        );
    END IF;

    -- 10. Founder Department-Wise Oversight Alert: Notify all Founders on every segregation
    FOR v_founder_id IN
    ...
    LOOP
        INSERT INTO public.in_app_notifications (
            ...
            format('%s - %s (%s) decomposed "%s" into %s subtasks (%s).', v_dept_name, COALESCE(v_actor.full_name, v_actor.email), v_actor.role, v_parent.title, v_created_count, v_child_names_summary),
            'TASK_SEGREGATED',
            format('/task/%s', p_parent_task_id)
        );
    END LOOP;
