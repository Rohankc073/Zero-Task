...
export default function FounderRecoveryScreen() {
  ...
  // Access validation: Strictly Super Admin only
  useEffect(() => {
    if (profile && !isSuperAdmin(profile)) {
      Alert.alert('Access Denied', 'Only Super Admin can access Founder Recovery.');
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error loading founder recovery requests:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleCompleteReset = async () => {
    ...
    Alert.alert(
      'Confirm Founder Password Reset',
      `Establish new password for Founder ${selectedItem.requester_name || selectedItem.email} (${selectedItem.company_name || 'Organization'})?\n\nAll existing sessions on all devices will be revoked immediately.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (res.error) {
                Alert.alert('Reset Failed', res.error.message || 'Could not complete founder password reset.');
                return;
              }

              Alert.alert('Success', 'Founder password reset completed successfully. All prior sessions have been revoked.');
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (res.error) {
        Alert.alert('Reject Failed', res.error.message || 'Could not reject founder recovery request.');
        return;
      }

      Alert.alert('Request Rejected', 'The founder password recovery request has been rejected.');
      ...
      >
        {loading && !refreshing ? (
          <View style={styles.centerBox}>
            ...
            <Text style={styles.loadingText}>Loading founder recovery requests...</Text>
          </View>
        ) : items.length === 0 ? (
          <View style={styles.emptyCard}>
            <Ionicons name="shield-checkmark-outline" size={48} color={Colors.textMuted} />
            <Text style={styles.emptyTitle}>No {statusFilter.toLowerCase()} founder recovery requests</Text>
            <Text style={styles.emptySubtitle}>
              {statusFilter === 'Pending'
                ? 'All founder password recovery requests have been resolved.'
              ...
              <View style={styles.cardHeader}>
                <View style={styles.userInfoRow}>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    <Text style={styles.userName}>{item.requester_name || 'Founder'}</Text>
                    <Text style={styles.userRole}>
                      Company Founder · {item.company_name || 'Enterprise'}
                    </Text>
                  </View>
                </View>
              ...
              {(item.status === 'Pending' || item.status === 'Approved') && (
                <View style={styles.cardActions}>
                  ...
                  >
                    ...
                    <Text style={styles.reviewBtnText}>Review & Set Founder Password</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
            ...
            <ScrollView style={styles.modalScroll} contentContainerStyle={{ padding: 20 }}>
              ...
              <View style={styles.identityCard}>
                ...
                <View style={{ flex: 1, marginLeft: 14 }}>
                  <Text style={styles.identityName}>{selectedItem.requester_name || 'Founder'}</Text>
                  <Text style={styles.identityRole}>Founder · {selectedItem.company_name || 'Organization'}</Text>
              ...
              <View style={styles.warningNotice}>
                ...
                <Text style={styles.warningText}>
                  As Super Admin, setting this password will establish new credentials for this Founder and revoke all current authentication sessions.
                </Text>
              </View>

              {/* Password Setting Form */}
              <Text style={styles.formSectionTitle}>Establish New Founder Password</Text>
              ...
              <Text style={styles.securityNote}>
                Credentials are automatically hashed via bcrypt and updated securely in PostgreSQL.
              </Text>
              ...
              <View style={styles.modalActionButtons}>
                ...
                >
                  {processing ? (
                    ...
                    <>
                      ...
                      <Text style={styles.completeBtnText}>Establish Founder Password</Text>
                    </>
                  )}
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={styles.rejectBtnText}>Reject Recovery Request</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          )}
      ...
      <Modal visible={rejectModalVisible} transparent animationType="fade">
        <View style={styles.rejectModalOverlay}>
          <View style={styles.rejectModalContent}>
            <Text style={styles.rejectModalTitle}>Reject Founder Recovery</Text>
            <Text style={styles.rejectModalSubtitle}>Provide an optional reason for the rejection:</Text>
            <TextInput
              style={styles.rejectInput}
              placeholder="e.g. Unverified request, Contacted via alternative channel..."
              placeholderTextColor={Colors.textMuted}
              value={rejectReason}
